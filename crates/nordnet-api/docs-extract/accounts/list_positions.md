# list_positions — Get account positions
...
| Query | include_instrument_loans optional | true if instrument loan positions should be included in the response. Defaults to false. | boolean | "false" |
| Query | include_intraday_limit optional | true if intraday limit should be included in the response. Defaults to false. | boolean | "false" |
...
|-----------|-------------|--------|
| 200 | Standard response for successful HTTP requests. | < Position > array |
...
- [Position](../_definitions/Position.md)
